...
def truss_main(file_path, nodes, members, load_casses, supports):
    """
    Perpares the inputs, and runs the optimization of the truss
    ...
    save_truss_cross_section_optimization_excel(file_path, nodes, members, load_casses, supports, A, u, q, vol)

def optimize_truss_cross_sections(nodes, members, load_casses, supports):
    """
    ...
    f, dof = _assign_loads_and_supports(nodes, load_casses, supports)
...
def _assign_loads_and_supports(nodes, load_casses, supports):
    """
    ...
    total_load = 0
    for case in load_casses:
        for load in case:
            total_load += abs(load[2]) + abs(load[3])
    if total_load == 0:
        total_load = 1

    f = [np.zeros(len(nodes) * 2)] * len(load_casses)
    for j, load_case in enumerate(load_casses):
...
def _plot_truss(nodes, members, a, threshold):
    ...
    THICKNESS_MULTIPLYER = 0.08

    for i in [i for i in range(len(a)) if a[i] >= threshold]:
        pos = nodes[members[i, [0, 1]].astype(int), :]
        plt.plot(pos[:, 0], pos[:, 1], linewidth=np.sqrt(a[i] * THICKNESS_MULTIPLYER), solid_capstyle='round')