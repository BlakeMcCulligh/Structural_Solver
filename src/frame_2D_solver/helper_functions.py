"""
Helper functions used for the analysis of a 3D frame.
"""
import numpy as np
import scipy.optimize as opt

from src.frame_3D_solver.cross_section_calculaters import angle, rect_hss, square_hss, tube_hss
import src.frame_2D_solver.fixed_end_reactions_calculaters as fer_calc

__author__ = "Blake McCulligh"
__copyright__ = "Copyright 2026 Blake McCulligh"
__credits__ = ["Blake McCulligh"]

__license__ = "MIT"
__version__ = "0.1.0b1"
__maintainer__ = "Blake McCulligh"
__email__ = "bmcculli@uwaterloo.ca"
__status__ = "Beta"

def part_D(nodes_support):
    """
    Builds lists of unreleased and released degree of freedom.

    :param nodes_support: ndarray. What nodes are supported in what DOFs.
    :return:
        nodes_dof_unknown: ndarray. A ndarray of the indices for the released DOFs.
        nodes_dof_known: ndarray. A ndarray of the indices for the unreleased DOFs.
    """
    n_nodes = nodes_support.shape[0]
    dof_indices = np.arange(n_nodes * 3).reshape(n_nodes, 3)

    D_known = []
    D_unknown = []

    for i in range(n_nodes):
        for j in range(3):

            if nodes_support[i,j]:
                D_known.append(dof_indices[i,j])
            else:
                D_unknown.append(dof_indices[i,j])

    D_known = np.array(D_known)
    D_unknown = np.array(D_unknown)

    return D_unknown, D_known

def prep_members(nodes_cord, members, members_releases, members_point_loads, members_dist_loads, num_c):
    """
    Prepares the members for an analysis. This function can be run at the start of an optimization and
    does not need to be rerun is only cross-sections are changed.


    :param nodes_cord: ndarray. Coordinates of the nodes [X, Y]
    :param members:  list. [i_node, j_node, material_index, set_cross_section_props]
    :param members_releases: list. What directions are released Bool.
                             [Dxi, Dyi, Ri, Dxj, Dyj, Rj]
    :param members_point_loads: list. Point loads applied to members.
                                [case, [[x], [[Px, Py, M]]]]
    :param members_dist_loads: list. Distributed loads applied to members.
                               [case, [[[x1, x2], [wx1, wx2, wy1, wy2]]]]
    :param num_c: int. Number of cases.
    :return:
        DOFs: ndarray. A 3D ndarray of the DOF indices for each member. shape: (# members, 6)
        L: ndarray. A ndarray of the lengths of the members. shape: (# members)
        member_unreleased_DOFs: list: 3D list of unreleased DOFs for each member.
                            shape: (# members, varys: # unreleased DOFs)
        member_released_DOFs: list: 3D list of released DOFs for each member. shape: (# members, varys: # released DOFs)
        T: ndarray: 4D ndarray of the transformation matrices for each member. shape: (# members, 6, 6)
    """

    L = _get_L(nodes_cord, members)
    members = members[:, :2]
    DOFs = _build_dof_vector(members)
    member_unreleased_DOFs, member_released_DOFs = _member_part_D(members_releases)
    T = _get_member_t(nodes_cord, members, L)
    point_loads = _assemble_point_loads(members_point_loads, num_c)
    dist_loads = _assemble_dist_loads(members_dist_loads, num_c)
    return DOFs, L, member_unreleased_DOFs, member_released_DOFs, T, point_loads, dist_loads

def _get_L(nodes_cord: np.ndarray, members: np.ndarray):
    """
    Builds an array of the lengths of the members.

    :param nodes_cord: ndarray. Coordinates of the nodes [X, Y]
    :param members: ndarray. [i_node, j_node, material_index, set_cross_section_props]
    :return:
        L: ndarray. A ndarray of the lengths of the members.
    """

    dx = nodes_cord[members[:, 1].astype(int) , 0] - nodes_cord[members[:, 0].astype(int) , 0]
    dy = nodes_cord[members[:, 1].astype(int) , 1] - nodes_cord[members[:, 0].astype(int) , 1]
    return np.sqrt(dx ** 2 + dy ** 2)

def _build_dof_vector(nodes: np.ndarray):
    """
    Returns the flattened list of global DOF indices for the supplied nodes.

    :param nodes: ndarray. A 3D ndarray of the indices of the nodes to get the DOF indices for. shape: (# members, 2)
    :return: ndarray. A 3D ndarray of the DOF indices for each member. shape: (# members, 6)
    """
    dofs = nodes[:, :, None] * 3 + np.arange(3)
    return dofs.reshape(nodes.shape[0], -1)

def _member_part_D(releases: np.ndarray):
    """
    Builds lists of unreleased and released degree of freedom indices for each member.

    :param releases: ndarray. 3D ndarray of booleans of whether a DOF is released. true = released, false = unreleased.
                     shape: (# members, 12)
    :return:
        unreleased: list: 3D list of unreleased DOFs for each member. shape: (# members, varys: # unreleased DOFs)
        released: list: 3D list of released DOFs for each member. shape: (# members, varys: # released DOFs)
    """

    idx = np.arange(6)
    unreleased = [idx[~r] for r in releases]
    released   = [idx[r]  for r in releases]
    return unreleased, released

def _get_member_t(nodes_cord, members, L: np.ndarray):
    """
    Builds an array of the transformation matrices for each member.

    :param nodes_cord: ndarray. Coordinates of the nodes [X, Y]
    :param members: list. [i_node, j_node, material_index, set_cross_section_props]
    :param L: ndarray. A ndarray of the lengths of the members. shape: (# members)
    :return: ndarray: 4D array of the transformation matrices for each member. shape: (# members, 6, 6)
    """

    i = members[:, 0].astype(int)
    j = members[:, 1].astype(int)
    Xi, Yi = nodes_cord[i].T
    Xj, Yj = nodes_cord[j].T
    # Local x-axis (direction cosines)
    x = np.stack([(Xj - Xi)/L, (Yj - Yi)/L], axis=1)
    # Initialize y, z
    n = len(members)
    y = np.zeros((n, 2))
    # Masks
    vertical   = np.isclose(Xi, Xj)
    horizontal = np.isclose(Yi, Yj)
    general    = ~(vertical | horizontal)
    # Vertical members
    up = Yj > Yi
    y[vertical & up]  = [-1, 0]
    y[vertical & ~up] = [ 1, 0]
    # Horizontal members
    y[horizontal] = [0, 1]
    # General members
    y[general, 0] = -x[general, 1]
    y[general, 1] = x[general, 0]
    # Direction cosine matrices
    dirCos = np.stack([x, y], axis=1)   # (n, 2, 2)
    # Build transformation matrices
    T = np.zeros((n, 6, 6))
    for k in range(2):
        T[:, k*3 : k*3+2, k*3 : k*3+2] = dirCos
        T[:, k * 3 + 2, k * 3 + 2] = 1.0
    return T

def get_global_fixed_end_reaction_vector(nodes_cord, members_dof, T: np.ndarray, D_unknown: np.ndarray,
                                         D_known: np.ndarray, fer_condensed_array, num_m: int, num_c: int):
    """
    Builds array of the global fixed end reaction vector for both fixed and released DOFs.

    :param nodes_cord: ndarray. Coordinates of the nodes [X, Y]
    :param members_dof: ndarray. A 3D ndarray of the DOF indices for each member. shape: (# members, 6)
    :param T: ndarray. 4D array of the transformation matrices for each member. shape: (# members, 6, 6)
    :param D_unknown: ndarray. A ndarray of the indices for the released DOFs.
    :param D_known: ndarray. A ndarray of the indices for the unreleased DOFs.
    :param fer_condensed_array: ndarray. Condensed local fixed end reaction vector. shape: (# members, # Cases, 6, 1)
    :param num_m: int. Number of members.
    :param num_c: int. Number of cases.
    :return:
        FER1: ndarray. An array of the released DOFs fixed end reactions.
        FER2: ndarray. An array of the unreleased DOFs fixed end reactions.
    """

    FER_array = _get_FER(fer_condensed_array, T)
    FER_stack = np.array([[np.asarray(FER_array[i][j], dtype=float).reshape(6)
                           for j in range(num_c)] for i in range(num_m)])  # shape: (# members, # Cases, 6)
    members_dof = np.asarray(members_dof)  # (num_m, 6)
    nDOF = len(nodes_cord) * 3
    FER_val = np.zeros((num_c, nDOF))
    dof_flat = members_dof.ravel()
    for j in range(num_c):np.add.at(FER_val[j], dof_flat.astype(int), FER_stack[:, j, :].ravel())
    FER_val = FER_val[..., None]
    FER1, FER2 = _part_force_vector(FER_val, D_unknown, D_known)
    return FER1, FER2

def _part_force_vector(FER_val: np.ndarray, D_unknown: np.ndarray, D_known: np.ndarray):
    """
    Partitions force vectors into 2 vectors one of released and one unreleased DOFs.

    :param FER_val: ndarray. A ndarray of forces to be partitioned.
    :param D_unknown: ndarray. A ndarray of the indices for the released DOFs.
    :param D_known: ndarray. A ndarray of the indices for the unreleased DOFs.
    :return:
        FER1: ndarray. An array of the released DOFs forces.
        FER2: ndarray. An array of the unreleased DOFs forces.
    """

    FER_val = np.asarray(FER_val)  # shape: (num_c, nDOF, 1)
    D_unknown = np.asarray(D_unknown, dtype=int)
    D_known = np.asarray(D_known, dtype=int)
    FER1 = FER_val[:, D_unknown, :]
    FER2 = FER_val[:, D_known, :]
    return FER1, FER2

def get_k_local_array(materials, members, members_CrossSectionProps, L: np.ndarray, log):
    """
    Builds an array of the local stiffness matrices for each member.

    :param materials: list. [E, G, nu, rho, fy]
    :param members: list. [i_node, j_node, material_index, set_cross_section_props]
    :param members_CrossSectionProps: ndarray. [A, I]
    :param L: ndarray. A ndarray of the lengths of the members. shape: (# members)
    :param log: bool. Used for debuging. Prints values when true.
    :return: 4D array of the local stiffness matrices for each member. shape: (# members, 6, 6)
    """

    E = materials[members[:, 2].astype(int), 0]
    G = materials[members[:, 2].astype(int), 1]
    A = members_CrossSectionProps[:, 0]
    I = members_CrossSectionProps[:, 1]

    n = len(L)
    # Precompute terms
    AE_L   = A * E / L
    EI_L  = E * I / L
    EI_L2 = E * I / L**2
    EI_L3 = E * I / L**3

    # Allocate
    k = np.zeros((n, 6, 6))
    # Fill matrix (vectorized indexing)
    # Row 0 / 3
    k[:, 0, 0] = AE_L
    k[:, 0, 3] = -AE_L
    k[:, 3, 0] = -AE_L
    k[:, 3, 3] = AE_L
    # Row 1 / 4
    k[:, 1, 1] = 12 * EI_L3
    k[:, 1, 2] = 6 * EI_L2
    k[:, 1, 4] = -12 * EI_L3
    k[:, 1, 5] = 6 * EI_L2
    k[:, 4, 1] = -12 * EI_L3
    k[:, 4, 2] = -6 * EI_L2
    k[:, 4, 4] = 12 * EI_L3
    k[:, 4, 5] = -6 * EI_L2

    # Row 2 / 5
    k[:, 2, 1] = 6 * EI_L2
    k[:, 2, 2] = 4 * EI_L
    k[:, 2, 4] = -6 * EI_L2
    k[:, 2, 5] = 2 * EI_L
    k[:, 5, 1] = 6 * EI_L2
    k[:, 5, 2] = 2 * EI_L
    k[:, 5, 4] = -6 * EI_L2
    k[:, 5, 5] = 4 * EI_L

    if log:
        print("local k: ", k)
    return k

def member_part_k_array(k_array: np.ndarray, R_unreleased_array: list or np.ndarray,
                        R_released_array: list or np.ndarray, numM: int):
    """
    Partitions the stiffness matrices into sub-matrices based on unreleased and released degree of freedom indices.

    :param k_array: ndarray. 4D array of the stiffness matrices for each member. shape: (# members, 6, 6)
    :param R_unreleased_array: list. 3D list of unreleased DOFs for each member.
                              shape: (# members, varys: # unreleased DOFs)
    :param R_released_array: list. 3D list of released DOFs for each member. shape: (# members, varys: # released DOFs)
    :param numM: int. Number of members.
    :return:  4 lists of sub-matrices based on the degrees of freedom.
              general shape: (# members, ndarray(varys, varys))
    """

    k11_array = []
    k12_array = []
    k21_array = []
    k22_array = []
    for i in range(numM):
        k = np.array(k_array[i])
        R_unreleased = np.array(R_unreleased_array[i], dtype=int)
        R_released = np.array(R_released_array[i], dtype=int)
        k11_array.append(k[R_unreleased, :][:, R_unreleased])
        k12_array.append(k[R_unreleased, :][:, R_released])
        k21_array.append(k[R_released, :][:, R_unreleased])
        k22_array.append(k[R_released, :][:, R_released])
    return k11_array, k12_array, k21_array, k22_array

def _get_FER(fer: np.ndarray, T: np.ndarray):
    """
    Returns the global fixed end reaction vectors for all members and cases

    :param fer: ndarray. The array of local fer vectors to be transformed. shape: (# members, # cases, 6, 1)
    :param T: ndarray. 4D array of the transformation matrices for each member. shape: (# members, 6, 6)
    :return: global fixed end reaction vectors for all members and cases
    """
    invT = T.transpose(0, 2, 1) # T⁻¹ = Tᵀ for orthogonal transformation matrices
    FER = invT[:, None] @ fer
    return FER

def _assemble_point_loads(members_point_loads, num_c):
    """
    Assembles the point load vectors for all members and cases

    :param members_point_loads:  list. Point loads applied to members. [case, [[x], [[Px, Py, M]]]]
    :param num_c: int. Number of cases
    :return: list. Point loads applied to each member.
             shape: (# members, # cases, # loads in case on member, 4: (x, Px, Py, M))
    """

    new_point_loads = []
    for i, load_cases in enumerate(members_point_loads):
        new_loads  = [None] * num_c
        for loads in load_cases:
            new_load  = []
            for j in range(len(loads[1][0])):
                new_load.append([loads[1][0][j]] + loads[1][1][j])
            # noinspection PyTypeChecker
            new_loads[int(loads[0])] = new_load
        for j in range(len(new_loads)):
            if new_loads[j] is None:
                # noinspection PyTypeChecker
                new_loads[j] = []
        new_point_loads.append(new_loads)
    return new_point_loads

def _assemble_dist_loads(members_dist_loads, num_c):
    """
    Assembles the distributed load vectors for all members and cases

    :param members_dist_loads: list. Distributed loads applied to members.
                              [case, [[[x1, x2], [wx1, wx2, wy1, wy2]]]]
    :param num_c: int. Number of cases
    :return: list. Distributed loads applied to each member.
             shape: (# members, # cases, varys, # loads in case on member: (x1, x2, wx1, wx2, wy1, wy2))
    """

    new_dist_loads = []
    for i, load_cases in enumerate(members_dist_loads):
        new_loads = [None] * num_c
        for loads in load_cases:
            new_load = []
            for j in range(len(loads[1])):
                new_load.append(loads[1][j][0] + loads[1][j][1])
            # noinspection PyTypeChecker
            new_loads[int(loads[0])] = new_load
        for j in range(len(new_loads)):
            if new_loads[j] is None:
                # noinspection PyTypeChecker
                new_loads[j] = []
        new_dist_loads.append(new_loads)
    return new_dist_loads

def get_member_fer_unc(members_L, point_loads: list, dist_loads: list, num_m: int, num_c: int):
    """
    Returns the local fixed end reaction vector for the members, ignoring the effects of end releases.

    :param members_L: ndarray. length of each member.
    :param point_loads: list. Point loads applied to each member.
                         shape: (# members, # cases, # loads in case on member, 4: (x, Px, Py, M))
    :param dist_loads: list. Distributed loads applied to each member.
                       shape: (# members, # cases, varys,
                       # loads in case on member: (x1, x2, wx1, wx2, wy1, wy2))
    :param num_m: int. Number of members.
    :param num_c: int. Number of cases.
    :return: ndarray. local fixed end reaction vector for the members. shape: (# members, # cases, 6, 1)
    """
    fer_unc = np.zeros((num_m, num_c, 6, 1))
    fer_unc = np.add(fer_unc, _get_fixed_end_reactions_point_load_array(members_L, point_loads))
    fer_unc = np.add(fer_unc, _get_fixed_end_reactions_dist_load_array(members_L, dist_loads))
    return fer_unc

def _get_fixed_end_reactions_point_load_array(members_L, point_loads: list):
    """
    Builds an array of the fixed end reactions for point loads.

    :param members_L: ndarray. length of each member.
    :param point_loads: list. Point loads applied to each member.
                        shape: (# members, # cases, varys: # loads in case on member, 4: (x, Px, Py, M))
    :return: ndarray. fixed end reactions for point loads. shape: (# members, # cases, 6, 1)
    """

    members_L = np.asarray(members_L)
    reactions = []
    for i, loadCases in enumerate(point_loads):
        L = float(members_L[i])
        reactionsMember = []
        for loads in loadCases:
            if loads is None or len(loads) == 0:
                reactionsMember.append(np.zeros((6, 1)))
                continue
            loads = np.asarray(loads)
            x  = loads[:, 0]
            Fx = loads[:, 1]
            Fy = loads[:, 2]
            M = loads[:, 3]
            R = np.zeros((6,))
            if np.any(Fx): R += fer_calc.point_load_x(Fx, x, L).sum(axis=0)
            if np.any(Fy): R += fer_calc.point_load_y(Fy, x, L).sum(axis=0)
            if np.any(M): R += fer_calc.moment(M, x, L).sum(axis=0)
            reactionsMember.append(R.reshape(6, 1))
        reactions.append(reactionsMember)
    return reactions

def _get_fixed_end_reactions_dist_load_array(members_L, dist_loads):
    """
    Builds an array of the fixed end reactions for distributed loads.

    :param members_L: ndarray. length of each member.
    :param dist_loads: list. Distributed loads applied to each member.
                       shape: (# members, # cases, varys,
                       varys: # loads in case on member: (x1, x2, wx1, wx2, wy1, wy2))
    :return: ndarray. fixed end reactions for distributed loads. shape: (# members, # cases, 6, 1)
    """

    reactions_array = []
    for i, load_cases in enumerate(dist_loads):
        L = members_L[i]
        reactions_member = []
        for j, loads in enumerate(load_cases):
            reactions = np.zeros((6, 1))
            for k, load in enumerate(loads):
                if not (load[2] == 0 and load[3] == 0):
                    reactions = reactions + fer_calc.distributed_load_x([load[2], load[3]], [load[0], load[1]], L)
                if not (load[4] == 0 and load[5] == 0):
                    reactions = reactions + fer_calc.distributed_load_y([load[4], load[5]], [load[0], load[1]], L)
            reactions_member.append(reactions)
        reactions_array.append(reactions_member)
    return reactions_array

def member_part_fer(fer: np.ndarray, R_unreleased: np.ndarray or list, R_released: np.ndarray or list,
                    num_m: int, num_c: int):
    """
    Partition fer into 2 sub matrices based on unreleased and released degree of freedom.

    :param fer: ndarray. local fixed end reaction vector for the members. shape: (# members, # cases, 6, 1)
    :param R_unreleased: ndarray or list. list: 3D list of unreleased DOFs for each member.
                        shape: (# members, varys: # unreleased DOFs)
    :param R_released: ndarray or list. list: 3D list of released DOFs for each member.
                      shape: (# members, varys: # released DOFs)
    :param num_m: int. Number of members.
    :param num_c: int. Number of cases.
    :return:
        fer1: list. fixed end reactions for each member for the unreleased DOFs.
              shape: (# members, # cases, varys: # unreleased DOFs, 1)
        fer2: list. fixed end reactions for each member for the released FODs.
              shape: (# members, # cases, varys: # released DOFs, 1)
    """

    fer1_array = []
    fer2_array = []
    for i in range(num_m):
        fer1_sub_array = []
        fer2_sub_array = []
        for j in range(num_c):
            fer1 = fer[i,j,R_unreleased[i]]
            fer2 = fer[i,j,R_released[i]]
            fer1_sub_array.append(fer1)
            fer2_sub_array.append(fer2)
        fer1_array.append(fer1_sub_array)
        fer2_array.append(fer2_sub_array)
    return fer1_array, fer2_array

def get_fer(members_releases, k12: list, k22: list, fer1: np.ndarray, fer2: np.ndarray, num_m: int, num_c: int):
    """
    Returns the condensed local fixed end reaction vector for all members and cases.

    :param members_releases: list. What directions are released Bool.
                             [Dxi, Dyi, Ri, Dxj, Dyj, Rj]
    :param k12: list. sub-matrix of the member local stiffness matrix. shape: (# members, varys)
    :param k22: list. sub-matrix of the member local stiffness matrix. shape: (# members, varys)
    :param fer1: ndarray. fixed end reactions for each member for the unreleased DOFs.
                 shape: (# members, # cases, varys: # unreleased DOFs, 1)
    :param fer2: ndarray. fixed end reactions for each member for the released FODs.
                 shape: (# members, # cases, varys: # released DOFs, 1)
    :param num_m: int. Number of members.
    :param num_c: int. Number of cases.
    :return: ndarray. Condensed local fixed end reaction vector. shape: (# members, # cases, 6, 1)
    """

    fer_condensed_array = []
    for i in range(num_m):
        fer_condensed_sub_array = []
        for j in range(num_c):
            fer_1 = np.array(fer1[i][j])
            fer_2 = np.array(fer2[i][j])
            k_12 = np.array(k12[i])
            k_22 = np.array(k22[i])
            fer_condensed = (fer_1 - k_12 @ k_22 @ fer_2)
            a = 0
            for DOF in members_releases[i]:
                if DOF:
                    fer_condensed = np.insert(fer_condensed, a, 0, axis=0)
                a += 1
            fer_condensed_sub_array.append(fer_condensed)
        fer_condensed_array.append(fer_condensed_sub_array)
    return fer_condensed_array

def get_parted_global_nodal_force_vector(nodes_loads, cases, D_unknown, D_known, num_n: int):
    """
    Builds the partitioned global nodal force vector.

    :param nodes_loads: list. [case, [Px, Py, M]]
    :param cases: list. Load Case Indexes.
    :param D_unknown: ndarray. A ndarray of the indices for the released DOFs.
    :param D_known: ndarray. A ndarray of the indices for the unreleased DOFs.
    :param num_n: int. Number of nodes
    :return: ndarray. Global nodal force vector partitioned into 2 vectors.
    """

    p_array = []
    for load_case_i in cases:
        p = np.zeros((num_n * 3, 1))
        for i in range(num_n):
            node_load = nodes_loads[i]
            local = np.zeros(3, dtype=float)
            for j, n in enumerate(node_load):
                if n[0] == load_case_i:
                    local = np.array(nodes_loads[i][int(load_case_i)][1])
            dofs = np.empty(len([i]) * 3, dtype=np.int64)
            local_ = np.arange(3, dtype=np.int64)
            for j, node in enumerate([i]):
                start = j * 3
                dofs[start:start + 3] = node * 3 + local_
            p[dofs, 0] += local
        p_array.append(p)
    return _part_force_vector(np.array(p_array), D_unknown, D_known)

def k_member_make_global(k_local_array, T_array):
    """
    Converts member stiffness matrices from the local coordinate system to the global coordinate system.

    :param k_local_array: ndarray. 4D array of the local stiffness matrices for each member. shape: (# members, 6, 6)
    :param T_array: ndarray: 4D array of the transformation matrices for each member. shape: (# members, 6, 6)
    :return: ndarray: 4D array of the global stiffness matrices for each member. shape: (# members, 6, 6)
    """

    k_member_global = []
    for i in range(len(T_array)):
        k_member_global.append(np.linalg.inv(T_array[i]) @ k_local_array[i] @ T_array[i])
    return k_member_global

def get_K_global(members_dof, k_global: np.ndarray, num_n: int, num_m:int):
    """
    Assembles the global stiffness matrix for the frame.

    :param members_dof: ndarray. A 3D ndarray of the DOF indices for each member. shape: (# members, 6)
    :param k_global: ndarray. 4D array of the global stiffness matrices for each member. shape: (# members, 6, 6)
    :param num_n: int. Number of nodes.
    :param num_m: int. Number of cases.
    :return: ndarray. Global stiffness matrix. shape: (# nodes * 3, # nodes * 3)
    """

    K = np.zeros((num_n * 3, num_n * 3))
    for i in range(num_m):
        K[np.ix_(members_dof[i].astype(int), members_dof[i].astype(int))] += np.asarray(k_global[i], dtype=float)
    return K

def partition_K_global(K_global: np.ndarray, R_unreleased: np.ndarray, R_released: np.ndarray):
    """
    Partitions the global stiffness matrix with respect to the frames supports.

    :param K_global: ndarray. Global stiffness matrix. shape: (# nodes * 3, # nodes * 3)
    :param R_unreleased: ndarray. A ndarray of the indices for the unreleased DOFs. (not supported)
    :param R_released: ndarray. A ndarray of the indices for the unreleased DOFs. (supported)
    :return: ndarray. 4 sub-matrices of the global stiffness matrix with respect to the frames supports.
    """

    K11 = (K_global[R_unreleased.astype(int), :][:, R_unreleased.astype(int)])
    K12 = (K_global[R_unreleased.astype(int), :][:, R_released.astype(int)])
    K21 = (K_global[R_released.astype(int), :][:, R_unreleased.astype(int)])
    K22 = (K_global[R_released.astype(int), :][:, R_released.astype(int)])
    return K11, K12, K21, K22

def get_D(K11: np.ndarray, K12:np.ndarray, P1_array:np.ndarray, FER1_array:np.ndarray, index_unsupported:np.ndarray,
          index_supported: np.ndarray, num_n: int, num_c:int, log):
    """
    Solves for the nodal displacement for the frame.

    :param K11: ndarray. Sub-matrices of the global stiffness matrix.
    :param K12: ndarray. Sub-matrices of the global stiffness matrix.
    :param P1_array: ndarray. Vector of the nodel loads.
    :param FER1_array: ndarray. An array of the released DOFs fixed end reactions.
    :param index_unsupported: ndarray. Indices of the unsupported DOFs.
    :param index_supported: ndarray. Indices of the supported DOFs.
    :param num_n: int. Number of nodes.
    :param num_c: int. Number of cases.
    :param log: bool. Used for debuging. Prints values when true.
    :return:
        D: ndarray. Array of the nodal displacements.
        DX: ndarray. Array of the nodel displacements in the X direction.
        DY: ndarray. Array of the nodel displacements in the Y direction.
        R: ndarray. Array of the nodel rotation.
    """

    D1_array = []
    D2 = np.zeros((len(index_supported), 1))
    for i in range(num_c):
        P1 = np.array(P1_array[i])
        FER1 = np.array(FER1_array[i])
        if K11.shape == (0, 0):
            D1_array.append([])
        else:
            try:
                D1_array.append(np.linalg.solve(K11, np.subtract(np.subtract(P1, FER1), np.matmul(K12, D2))))
            except np.linalg.LinAlgError:

                # handeling if the structure is unstable

                print("Structure is unstable")

                return None, None, None, None

    D_array = _assemble_D_array(D1_array, D2, index_supported, index_unsupported, num_n, num_c)

    if log:
        print("D_array: ", D_array)

    DX_array, DY_array, R_array = _get_node_direction_deflections(D_array, num_n, num_c)

    if log:
        print("DX_array: ", DX_array)
        print("DY_array: ", DY_array)
        print("RX_array: ", R_array)


    return D_array, DX_array, DY_array, R_array

def _assemble_D_array(D1_array, D2, index_supported, index_unsupported, num_n: int, num_c: int):
    """

    :param D1_array: ndarray. Array of displacements for all unsupported DOFs.
    :param D2: ndarray. Array of displacements for all supported DOFs.
    :param index_supported: ndarray. Indices of the supported DOFs.
    :param index_unsupported: ndarray. Indices of the unsupported DOFs.
    :param num_n: int: Number of nodes.
    :param num_c: int: Number of cases.
    :return: ndarray. Array of the nodal displacements.
    """

    D_array = []
    for a in range(num_c):
        D1 = np.array(D1_array[a])
        D = np.zeros((num_n * 3, 1))
        for j in range(num_n):
            for i in range(3):
                if j * 3 + i in index_supported:
                    D[(j * 3 + i, 0)] = D2[index_supported.tolist().index(j * 3 + i), 0]
                else:
                    D[(j * 3 + i, 0)] = D1[index_unsupported.tolist().index(j * 3 + i), 0]
        D_array.append(D)
    return D_array

def _get_node_direction_deflections(D_array, num_n: int, num_c: int):
    """
    Gets the nodal deflection in each direction.

    :param D_array: ndarray. Array of the nodal displacements.
    :param num_n: int: Number of nodes.
    :param num_c: int: Number of cases.
    :return:
        DX: ndarray. Array of the nodel displacements in the X direction.
        DY: ndarray. Array of the nodel displacements in the Y direction.
        R: ndarray. Array of the nodel rotation displacements.
    """

    DX_array = []
    DY_array = []
    R_array = []
    for i in range(num_c):
        DX_case = []
        DY_case = []
        R_case = []
        for j in range(num_n):
            DX_case.append(D_array[i][j * 3 + 0, 0])
            DY_case.append(D_array[i][j * 3 + 1, 0])
            R_case.append(D_array[i][j * 3 + 2, 0])
        DX_array.append(DX_case)
        DY_array.append(DY_case)
        R_array.append(R_case)
    return np.array(DX_array), np.array(DY_array), np.array(R_array)

def get_member_direction_deflections(members, DX_array: np.ndarray, DY_array: np.ndarray,R_array: np.ndarray,
                                     num_m: int, num_c: int):
    """
    Gets the global end deflections for each member.

    :param members:  list. [i_node, j_node, material_index, set_cross_section_props]
    :param DX_array: ndarray. Array of the nodel displacements in the X direction.
    :param DY_array: ndarray. Array of the nodel displacements in the Y direction.
    :param R_array: ndarray. Array of the nodel rotation displacements.
    :param num_m: int. Number of members.
    :param num_c: int. Number of cases.
    :return: ndarray. Array of the global end deflections for each member. shape: (# cases, # members, 6)
    """

    D_member_array = []
    for i in range(num_c):
        D_sub = []
        for j in range(num_m):
            Dx1 = DX_array[i, members[j, 0].astype(int)]
            Dy1 = DY_array[i, members[j, 0].astype(int)]
            R1 = R_array[i, members[j, 0].astype(int)]
            Dx2 = DX_array[i, members[j, 1].astype(int)]
            Dy2 = DY_array[i, members[j, 1].astype(int)]
            R2 = R_array[i, members[j, 1].astype(int)]
            D_sub.append(np.array([Dx1, Dy1, R1, Dx2, Dy2, R2]))
        D_member_array.append(D_sub)
    return np.array(D_member_array)

def get_d(T_array: np.ndarray, D_array: np.ndarray, num_m: int, num_c: int):
    """
    Transforms the end deflection array for each member from the global coordinate system into the local
    coordinate system.

    :param T_array: ndarray. 4D ndarray of the transformation matrices for each member. shape: (# members, 6, 6)
    :param D_array: ndarray. Array of the global end deflections for each member. shape: (# cases, # members, 6)
    :param num_m: int. Number of members.
    :param num_c: int. Number of cases.
    :return: ndarray. Array of the local end deflections for each member. shape: (# cases, # members, 6)
    """

    d = []
    for i in range(num_c):
        d_sub = []
        for j in range(num_m):
            d_sub.append(T_array[j] @ D_array[i][j])
        d.append(d_sub)
    return np.array(d)

def get_F(T_array: np.ndarray, f_array: np.ndarray, num_m: int, num_c:int):
    """
    Gets the global forces acting at each end of the members.

    :param T_array: ndarray. 4D ndarray of the transformation matrices for each member. shape: (# members, 6, 6)
    :param f_array: ndarray. Array of the local forces acting at the ends of the members.
                    shape: (# cases, # members, 6)
    :param num_m: int. Number of members.
    :param num_c: int. Number of cases.
    :return: ndarray. Array of the global forces acting at the ends of the members. shape: (# cases, # members, 6)
    """

    F = []
    for i in range(num_c):
        F_sub = []
        for j in range(num_m):
            F_sub.append(np.linalg.inv(T_array[j]) @ f_array[i, j])
        F.append(F_sub)
    return np.array(F)

def get_f(k_array: np.ndarray, d_array: np.ndarray, fer_array: list, num_m: int, num_c: int):
    """
    Gets the local forces acting at each end of the members.

    :param k_array: ndarray. 4D array of the local stiffness matrices for each member. shape: (# members, 6, 6)
    :param d_array: ndarray. Array of the local end deflections for each member. shape: (# cases, # members, 6)
    :param fer_array: list. Condensed local fixed end reaction vector. shape: (# members, # cases, 6, 1)
    :param num_m: int: Number of members.
    :param num_c: int: Number of cases.
    :return: ndarray. Array of the local forces acting at the ends of the members. shape: (# cases, # members, 6)
    """

    f = []
    for i in range(num_c):
        f_sub = []
        for j in range(num_m):
            f_sub.append(k_array[j] @ d_array[i, j] + fer_array[j][i])
        f.append(f_sub)
    return np.array(f)

def get_weight(materials, members, members_L, members_cross_section_props):
    """
    Gets the weight of each member of the frame.

    :param materials:  list. [E, G, nu, rho, fy]
    :param members: list. [i_node, j_node, material_index, set_cross_section_props]
    :param members_L: ndarray. length of each member.
    :param members_cross_section_props: ndarray. [A, I]
    :return: ndarray. Array of the weights of each member of the frame.
    """

    return materials[members[:,2].astype(int),3] * members_cross_section_props[:, 0] * members_L

def get_reactions(nodes_support, nodes_loads, members, members_releases, F_array, num_c:int, num_m:int, num_n:int):
    """
    Gets the reactions for the frame. If an DOF is not supported the reaction is 0.

    :param nodes_support: list. [support_DX, support_DY, support_R]
    :param nodes_loads:  list. [case, [Px, Py, M]]
    :param members: list. [i_node, j_node, material_index, set_cross_section_props]
    :param members_releases: list. What directions are released Bool.
                             [Dxi, Dyi, Ri, Dxj, Dyj, Rj, ]
    :param F_array: ndarray. Array of the global forces acting at the ends of the members.
                    shape: (# cases, # members, 6)
    :param num_c: int: Number of cases.
    :param num_m: int: Number of members.
    :param num_n: int: Number of nodes.
    :return: ndarray. Array of the reactions at the supports of the frame. shape: (# cases, # nodes, 3)
    """

    reactions = []
    for i in range(num_c):
        reactions_sub = []
        for j in range(num_n):
            r = [0.0] * 3
            for s in range(3):
                if nodes_support[i][s]:
                    for k in range(num_m):
                        if members[k, 0] == j:
                            if not members_releases[k][s]: r[s] += F_array[i, k, s, 0]
                        elif members[k, 1] == j:
                            if not members_releases[k][s + 3]: r[s] += F_array[i, k, s + 3, 0]
                    for joint_load_case in nodes_loads[j]:
                        if joint_load_case[0] == i:
                            r[s] += joint_load_case[1][s]
            reactions_sub.append(r)
        reactions.append(reactions_sub)
    return reactions

"""-----------------Optimization-----------------------------"""

# noinspection PyUnusedLocal
def _cost(D, DX, DY, R, Weight, Reactions, cost_function):
    return eval(cost_function)

def get_cost(X, constants):
    """
    Gets the _cost for the current optimization variables.

    :param X: list. Optimization variables.
    :param constants: list. arguments needed to calculate the _cost: [cost_function, frame, member_group, member_group_type,
                     weight_needed, reactions_needed, log]
    :return: float. _cost
    """
    #log = True
    (frame, _get_cross_section_props, cost_function, member_group, member_group_type,
     weight_needed, reactions_needed, log) = constants

    if log: print("X in: ", X)

    cross_section_props = _get_cross_section_props(X, member_group, member_group_type)
    if log: print("A Variable cross section properties: ", cross_section_props)

    j = 0
    for i in range(len(frame.members_cross_section_props)):
        if not frame.members[i][3]:
            frame.members_cross_section_props[i] = cross_section_props[j]
            j += 1
    if log: print("Cross Section Properties: ", frame.members_cross_section_props)

    D, DX, DY, R, weight, reactions = (
        frame.AnalysisLinear(weight_needed, reactions_needed, log))
    cost = _cost(D, DX, DY, R, weight, reactions, cost_function)
    #print("Cost: ", cost)
    return cost

def chack_inputs(members, member_group: list, member_group_type: list):
    """
    Checks if the input lists for the optimization are valid.

    :param members: list. [i_node, j_node, material_index, set_cross_section_props]
    :param member_group: list. list of indices of member groups for non set members to be assigned to.
    :param member_group_type: list. List of cross-section types for each member group to be assigned.
    """

    number_not_set_members = 0
    for member in members:
        if not member[3]:
            number_not_set_members += 1
    if len(member_group) != number_not_set_members:
        raise Exception('Number of not set members is not equal to number of members assigned to a group.')
    if max(member_group) + 1 != len(member_group_type):
        raise Exception('Number of member groups does not equal number of groups members are assigned to')

def get_num_variables(member_group_type: list):
    """
    Gets the number of variables to be in the optimization problem.

    :param member_group_type: list. List of cross-section types for each member group to be assigned.
    :return:
        numVariables: int, number of variables in the optimization.
    """

    num_variables = 0
    for t in member_group_type:
        if t == "Angle":
            num_variables += 3
        elif t == "RectHSS":
            num_variables += 3
        elif t == "SquareHSS":
            num_variables += 2
        elif t == "TubeHSS":
            num_variables += 2
        else:
            raise Exception('member group type is not a valid cross section type. '
                            'Chose one of the following: Angle, RectHSS, SquareHSS, TubeHSS')
    return num_variables

def get_bounds(lower_bound: list or float, upper_bound: list or float, num_variables: int):
    """
    Sets up the scipy optimization bounds.

    :param lower_bound: list or float. Lower bound on the optimization variables.
                        if list must be length of number of variables.
    :param upper_bound: list or float. Upper bound on the optimization variables.
                        if list must be length of number of variables.
    :param num_variables: int, Number of variables to be in the optimization problem.
    :return: Scipy optimization bounds.
    """

    if isinstance(lower_bound, float):
        lower_bound = [lower_bound] * num_variables
    if isinstance (upper_bound, float):
        upper_bound = [upper_bound] * num_variables
    return opt.Bounds(lower_bound, upper_bound)

def get_cross_section_props(X, member_groups, member_group_type):
    """
    Gets the cross-section properties of the cross-section being optimized from the optimization variables.

    :param X: list: Optimization variables.
    :param member_groups: list. list of indices of member groups for non set members to be assigned to.
    :param member_group_type: list. List of cross-section types for each member group to be assigned.
    :return: list: Cross-section properties for the optimization cross-sections.
             shape: (# cross-sections being optimized, 4
    """

    group_properties = []
    j = 0
    for t in member_group_type:
        if t == "Angle":
            A = angle.get_A(X[j], X[j + 1], X[j + 2])
            I = angle.get_Ix(X[j], X[j + 1], X[j + 2])
            group_properties.append([A, I])
            j += 3
        elif t == "RectHSS":
            A = rect_hss.get_A(X[j], X[j + 1], X[j + 2])
            I = rect_hss.get_Ix(X[j], X[j + 1], X[j + 2])
            J = rect_hss.get_J(X[j], X[j + 1], X[j + 2])
            group_properties.append([A, I])
            j += 3
        elif t == "SquareHSS":
            A = square_hss.get_A(X[j], X[j + 1])
            I = square_hss.get_I(X[j], X[j + 1])
            group_properties.append([A, I])
            j += 2
        elif t == "TubeHSS":
            A = tube_hss.get_A(X[j], X[j + 1])
            I = tube_hss.get_I(X[j], X[j + 1])
            group_properties.append([A, I])
            j += 2

    member_properties = []
    for member_g in member_groups:
        member_properties.append(group_properties[member_g])
    return member_properties