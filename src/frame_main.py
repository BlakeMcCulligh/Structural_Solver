...
from src.excel_analysis import frame_3d_global_opt, frame_3d_opt_temp, frame_3d_local_opt, frame_2d_global_opt, \
    frame_2d_local_opt, frame_2d_opt_temp
...
class FrameMain(tk.Frame):
    ...
    def _create_top_menu(self) -> tk.Menu:
        ...
        excel_menu.add_command(label='2D Frame Global Optimization', command=self._ex_2d_frame_global_opt)
        excel_menu.add_command(label='2D Frame Local Optimization', command=self._ex_2d_frame_local_opt)
        ...
        ex_temp_menu.add_command(label='2D Frame Global Optimization', command=self._ex_2d_frame_global_opt_temp)
        ex_temp_menu.add_command(label='2D Frame Local Optimization', command=self._ex_2d_frame_local_opt_temp)
    ...
    def _ex_2d_frame_global_opt(self) -> None:
        """
        Gets the selected Excel file and then runs an optimizes of the cross-sections of a 2D frame in the global space.
        """

        file_path = select_file_gui([("Excel Files", "*.xlsx")])

        if file_path is not None:
            frame = Frame2D(self.controller, None)
            frame_2d_global_opt(file_path, self.controller, frame)

    def _ex_2d_frame_local_opt(self) -> None:
        """
        Gets the selected Excel file and then runs an optimizes of the cross-sections of a 2D frame in the local space.
        """
        ...
        if file_path is not None:
            frame = Frame2D(self.controller, None)
            frame_2d_local_opt(file_path, self.controller, frame)

    @staticmethod
    def _ex_2d_frame_global_opt_temp() -> None:
        """
        Exports a template Excel file forff the 2D frame global analysis.
        """

        file_path = get_new_file_path(".xlse", [("Excel Files", "*.xlsx")])

        if file_path is not None:
            frame_2d_opt_temp(file_path, "Global")

    @staticmethod
    def _ex_2d_frame_local_opt_temp() -> None:
        """
        Exports a template Excel file for the 2D frame local analysis.
        """

        file_path = get_new_file_path(".xlse", [("Excel Files", "*.xlsx")])

        if file_path is not None:
            frame_2d_opt_temp(file_path, "Local")
