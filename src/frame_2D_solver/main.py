"""
Holds the 2D frame solver object and handles everything to do with the solving of 2D frames.
"""
...
import src.frame_2D_solver.helper_functions as hf
...
class Frame2D_Solver:
    """
    Object that handles the solving of a 2D frame.
    """
    def __init__(self) -> None:
        """
        Initializes the 2D frame solver object.
        """
        ...
        # Node Lists
        self.nodes_cord: np.ndarray | list = [] # shape: (# Nodes, 2)
        self.nodes_loads: np.ndarray | list = [] # shape: (# Nodes, # Cases, 3: [Px, Py, M])
        self.nodes_support: np.ndarray | list = [] # shape: (# Nodes, 3)
        ...
        self.members_cross_section_props = [] # shape: (# Members, 2: [A, I])
        self.members_releases = [] # shape: (# Members, 6)
        self.members_dof = [] # shape: (# Members, 6)
        ...
        # shape: (# Members, # Cases, 2: [Case Index, 2: [# loads: locations, # loads: [3]]])
        self.members_point_loads = []
        # shape: (# Members, # Cases, 2: [Case Index, # Loads:[2: locations,4:loads]])
    ...
    def AddNode(self, X: float, Y: float) -> None:
        """
        ...
        self.nodes_cord.append([X, Y])
        self.nodes_loads.append([])
        self.nodes_support.append([False, False, False])
    ...
    def AddMember(self, i_node: int, j_node: int, material_index: int, set_cross_section_props: bool, A: float,
        ...
        :param I: float. Moment of Inertia in the y direction.
        """

        self.members.append([i_node, j_node, material_index, set_cross_section_props])
        self.members_cross_section_props.append([float(A), float(I)])
    ...
    def AddSupport(self, node_index: int, support_DX: bool = False, support_DY: bool = False,
                   support_R: bool = False) -> None:
        """
        ...
        :param support_R: bool. Indicates if rotation should be supported.

        """

        self.nodes_support[int(node_index)] = [support_DX, support_DY, support_R]

    def AddReleases(self, member_index: int, Dxi: bool = False, Dyi: bool = False,
                    Ri: bool = False, Dxj: bool = False, Dyj: bool = False, Rj: bool = False) -> None:
        """
        ...
        :param Ri: bool. Indicates if the i node in the rotation direction should be released.
        ...
        :param Rj: bool. Indicates if the j node in the rotation direction should be released.
        """

        self.members_releases[member_index] = [Dxi, Dyi, Ri, Dxj, Dyj, Rj]

    def AddNodeLoad(self, node_index: int, Px: float = 0, Py: float = 0, M: float = 0, case: int = 0) -> None:
        """
        ...
        if not case_found:
            ...
            self.nodes_loads[node_index].append([case, [Px, Py, M]])

            if case not in self.cases: self.cases.append(case)

    def AddMemberPointLoad(self, member_index: int, x: float, Px: float = 0, Py: float = 0, M: float = 0,
                           case: int = 0) -> None:
        """
        ...
        for loads_case in self.members_point_loads[int(member_index)]:
            ...
            if case_index == case:
                ...
                if not load_found:
                    loadsX.append(x)
                    loadsMag.append([Px, Py, M])

        if not case_found:

            self.members_point_loads[int(member_index)].append([case, [[x], [[Px, Py, M]]]])

            if case not in self.cases: self.cases.append(case)

    def AddMemberDistLoad(self, member_index: int, x1: float, x2: float, wx1: float = 0, wx2: float = 0,
                          wy1: float = 0, wy2: float = 0, case: int = 0) -> None:
        """
        ...
        for loads_case in self.members_dist_loads[member_index]:
            ...
            if case_index == case:
                case_found = True
                loads.append([[x1, x2], [wx1, wx2, wy1, wy2]])

        if not case_found:

            self.members_dist_loads[member_index].append([case, [[[x1, x2], [wx1, wx2, wy1, wy2]]]])
    ...
    def AnalysisLinear(self, get_weight: bool = False, get_reactions: bool = False,
        ...
        :return:
            ...
            R: ndarray. Node rotation deflection.
        ...
        if isinstance(self.members_L, np.ndarray) and isinstance(self.members_T, np.ndarray):
            ...
            D, DX, DY, R = hf.get_D(K11, K12, P1, FER1, self.nodes_dof_unknown, self.nodes_dof_known,
                                                 num_n, num_c, log)
            ...
            if D is not None:
                ...
                if get_reactions:
                    D_members = hf.get_member_direction_deflections(self.members, DX, DY, R, num_m, num_c)
            ...
            return D, DX, DY, R, weight, reactions
        else:
    ...
    def optimize_global(self, memberGroup: list, memberGroupType: list, lowerBound: list | float, upperBound: list | float,
                        costFunction: str, getWeight: bool = False, getReactions: bool = False, log: bool =False):
        """
        ...
        constants = [self, hf.get_cross_section_props, costFunction,
                     memberGroup, memberGroupType, getWeight, getReactions, log]
    ...
    def optimize_local(self, memberGroup: list, memberGroupType: list, lowerBound: list | float,
                       ...
                       getReactions: bool = False, log: bool =False):
        """
